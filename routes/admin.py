...
def create_scenario():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            scenario_manager.create_scenario(complete_scenario_data, category)
...
@admin_bp.route('/export-user-statistics', methods=['POST'])
@login_required
@instructor_required
def export_user_statistics():
    ...
    for user in users:
        total_sessions = user.training_sessions.count()
        completed_sessions = user.training_sessions.filter_by(status='completed').count()
        completion_rate = (completed_sessions / total_sessions * 100) if total_sessions > 0 else 0

        completed = user.training_sessions.filter_by(status='completed').all()
        avg_score = sum(s.score for s in completed if s.score) / len(completed) if completed else 0

        best_score = max((s.score for s in user.training_sessions.all() if s.score), default=0)

        last_session = user.training_sessions.order_by(TrainingSession.completed_at.desc()).first()
        last_activity = last_session.completed_at.strftime('%Y-%m-%d %H:%M') if last_session and last_session.completed_at else 'N/A'
